...
void cWeaponsToolkit::onImportDirectoryChanged(wxCommandEvent& evt) {
	...
	int size_needed = MultiByteToWideChar(CP_UTF8, 0, tmpDir.c_str(), (int)tmpDir.size(), NULL, 0);
	std::wstring wide(size_needed, 0);
	MultiByteToWideChar(CP_UTF8, 0, tmpDir.c_str(), (int)tmpDir.size(), &wide[0], size_needed);
...
void cWeaponsToolkit::onCreateWeaponNextButtonChanged(wxCommandEvent& evt)
{
	if (!generatedWeapon->getValidWeaponModelFound()) {
		wxMessageBox(wxT("No valid weapon model found."), wxT("xTools"), wxICON_ERROR);
	}
	else {
...
void cWeaponsToolkit::onRemoveComponent(wxCommandEvent& evt)
{
	...
	try {
		...
		wxMessageBox("No weapon component to remove.", wxT("xTools"), wxICON_EXCLAMATION);
	}
}
...
void cWeaponsToolkit::onExportButtonPressed(wxCommandEvent& evt)
{
	if (!generatedWeapon->getValidWeaponModelFound())
	{
		wxMessageBox("Failed to export weapon, valid model files were not found.", wxT("xTools"), wxICON_ERROR);
		return;
	}
	...
	if (exportDirectory == "") {
		wxMessageBox("Failed to export weapon, no export directory found.", wxT("xTools"), wxICON_ERROR);
		return;
	}
	...
	if (generatedWeapon->getWeaponName() == "") {
		wxMessageBox("Failed to export weapon, no weapon ID found.", wxT("xTools"), wxICON_ERROR);
		return;
	}
	...
	if (weapon_result == 0) {
		...
		wxMessageBox("Failed to export weapon, failed to create folder.", wxT("xTools"), wxICON_ERROR);
		return;
	}
	...
	if (manifest.is_open())
	{
		...
		manifest << "games {'gta5'}\n";
		manifest << "description 'Add-on weapon generated using xTools'\n\n";
	...
	wxMessageBox(std::string("Successfully exported weapon to: ") + exportDirectory.c_str() + "\\" + generatedWeapon->getWeaponName(), wxT("xTools"), wxICON_INFORMATION);
}
...
cWeaponsToolkit::cWeaponsToolkit() : wxFrame(nullptr, wxID_ANY, "xTools", wxPoint((wxSystemSettings::GetMetric(wxSYS_SCREEN_X) / 2) - (800/2), (wxSystemSettings::GetMetric(wxSYS_SCREEN_Y) / 2) - (500 /2)), wxSize(windowWidth, windowHeight))
{
